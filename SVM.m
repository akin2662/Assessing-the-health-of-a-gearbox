...
labels = [zeros(1, numFilesPerCondition(1)),zeros(1, numFilesPerCondition(2)),zeros(1, numFilesPerCondition(3)),zeros(1, numFilesPerCondition(4)), ones(1, numFilesPerCondition(5))]; 
labels = labels';
...
SVM_model = fitcsvm(features_pca, labels);

cross_val_model = crossval(SVM_model);
...
predictions = predict(SVM_model, features_test_pca);
comp_time = toc;
...
title('SVM Training Data - Healthy vs Faulty');
...
xlabel('Principal Component 1');
ylabel('Principal Component 2');
title('SVM Test Data - Preditions');
...
title('Confusion matrix SVM-test data');
